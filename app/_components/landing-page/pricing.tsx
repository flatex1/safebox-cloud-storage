      ...
      <div className="max-w-screen-xl mx-auto px-4 lg:px-8">
        <div className="max-w-xl mx-auto text-center">
          ...
          <p className="text-foreground/60 mt-6 md:text-lg">
            Получите необходимые функции сегодня с возможностью обновления по
...
function PricingCard({
  ...
  const handleSubscription = async () => {
    if (!user || !organization) {
      toast.error("Пожалуйста, войдите в систему или выберите организацию");
      return;
    }
    ...
    try {
      ...
      let planType = "free";
      if (plan === "Про") planType = "pro";
      if (plan === "Бизнес") planType = "business";
  ...
  return (
    <motion.div
      ...
      transition={{
        duration: 0.5,
        delay: plan === "Про" ? 0.2 : plan === "Бизнес" ? 0.4 : 0,
      }}
      key={plan}
      className={cn(
    ...
    >
      ...
      <div className="flex grow items-end py-4">
        {plan === "Про" ? (
          <Button className="w-full h-10" onClick={handleSubscription}>
            {cta}
          </Button>
        ) : (
          <Button
